...
12. **UI 카피는 격식체(`-습니다`/`-하세요`).** 빈상태·에러상태·버튼·confirm·placeholder 전부. 반말(`-어/-야/-까?`)은 **에이전트가 사용자와 대화할 때만** 쓰고 UI 문자열엔 절대 쓰지 않는다. 기존 레퍼런스: `"세션이 없습니다."`, `"페이지를 찾을 수 없습니다."`, `"~/.claude/ 를 스캔해 … 파일을 변경하지 않습니다."`.
...
**코드 변경이 포함된 커밋** 전에 `bash scripts/check.sh` 를 실행하여 전체 검증:
...
문서만 변경한 커밋(`.md`, 설정 파일 등)은 check.sh 불필요.
